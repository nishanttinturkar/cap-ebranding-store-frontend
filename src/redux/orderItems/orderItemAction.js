...
import {
  FETCH_ORDERITEMS_REQUEST,
  FETCH_ORDERITEMS_SUCCESS,
  FETCH_ORDERITEMS_FAILURE,
  ADD_ORDERITEMS_REQUEST,
} from "./orderItemTypes";
...
export const getItemsByOrderId = (ordId) => {
  return (dispatch) => {
    let service = new OrderService();
    service
      .getOrderItemsbyId(ordId)
      .then((res) => {
        console.log("Itemssss", res.data);
        dispatch(fetchOrderItemsSuccess(res.data));
      })
      .catch((error) => {
        console.log(error);
      });
